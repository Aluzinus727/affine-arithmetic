...
class LinearExpression:
    ...
    def to_intervals(cls, intervals):
        ...
        for i in range(n):
            ...
            linear_vars.append(LinearExpression(a,0, new_x))
    ...
    def cheby_convex(self, f=lambda x: x**2, df_proj=lambda m: m/2, convex=False, concave=False):
        ...
        if convex: 
            if m >= (f(ub) - f(lb)) / (ub - lb):
                upper = m_static*self
                upper.b += (f(lb) - m_static * lb)
            else:
                upper = m_static*self 
                upper.b += (f(ub) - m_static * ub)
            ...
            if m >= (f(ub) - f(lb)) / (ub - lb):
                lower = m_static*self 
                lower.b += (f(ub) - m_static * ub)
            else:
                lower = m_static*self 
                lower.b += (f(lb) - m_static * lb)
...
class LinearEnvelope:
    ...
    def get_range(self):
        return Interval(self.lower(Affine.x_ref).lb(), self.upper(Affine.x_ref).ub())
